...
#+end_src
** Inside std::forward
The =std::forward= is used to forward an incoming reference to another function as the same type.
Therefore, [[id:200174fb-050d-4ec2-86e4-696427dc74ac][lvalue references]] and [[id:78938fd8-6e6e-4c4d-b899-96cccda80177][rvalue references]] are forward as such.

- If =T= is of value type, a =T&&= is the result of the expression
- If =T= is of lvalue reference type, a =T & &&= is the result of the expression
- If =T= is of rvalue reference type, a =T && &&= is the result of the expression


A =T & &&= is a rvalue reference to an lvalue reference, which collapse to a simple lvalue reference.
A =T && &&= is a rvalue reference to an lvalue reference, which collapse to a simple rvale reference.

#+CAPTION: Example implementation for std::forward
#+NAME: lst:std-forward-for-lvalue-references
#+begin_src c++
  template <typename T>
...
** Iterator_traits<>
=std::iterator_traits= is used implement algorithms only based on this "interface".
Therefore, if a type provides all required properties, it can be used as an iterator.

#+CAPTION: Example usage of iterator_traits
#+NAME: lst:example-usage-of-iterator_traits
#+begin_src c++
...
#+CAPTION: Example usage std::condition_variable
#+NAME: lst:example-usage-std-condition_variable
#+begin_src c++
...
** cmake
When the flag is =PUBLIC=, a libraries or executables which links against it will inherit the properties.
Therefore, libraries should be =PUBLIC= and executables =PRIVATE= because you cannot link against an executable.


#+CAPTION: Example for CMakeLists.txt
#+NAME: lst:example-for-cmakelists-txt
#+begin_src sh
...
#+CAPTION: Build cmake project
#+NAME: lst:build-cmake-project
#+begin_src sh
  mkdir build
  cd build
  cmake ..
...
** Project Layout
#+LATEX: {
[[file:img/cpp_project_layout_app.png]]
#+LATEX: \captionof{figure}{Recomended project layout for an application}\label{fig:recomended-project-layout-for-an-application}
#+LATEX: }


#+LATEX: {
[[file:img/cpp_project_layout_library.png]]
#+LATEX: \captionof{figure}{Recomended project layout for a library}\label{fig:recomended-project-layout-for-a-library}
#+LATEX: }
